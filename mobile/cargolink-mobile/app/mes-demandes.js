...
export default function MesDemandes() {
  ...
  const charger = async () => {
    ...
    console.log('Erreur charger mes-demandes:',err.response?.status,err.response?.data);
  }
  };
          ...
          <View style={styles.card}>
            ...
            {item.contact_debloque ? (
              ...
              <View style={styles.commissionBox}>
                <Text style={styles.commissionText}>Payez {Math.round(item.montant_final * 0.07).toLocaleString()} FCFA sur le numero EXDIVIA SARL pour recevoir le contact</Text>
              </View>
            ) : null}
          </View>
        )}
